...
int fts_fwupg_get_vendorid(struct fts_ts_data *ts_data, u16 *vid)
{
    ...
    FTS_INFO("focal ctpm_LockDownInfo_get_from_boot, tp_lockdown_info=%s",
...
static int fts_fwupg_get_fw_file(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < FTS_GET_VENDOR_ID_NUM; i++) {
        ...
        if (vendor_id == fw->vendor_id) {
            ...
            if (r_buf[1] == 0x36) {
                /* upgrade C3C_FT5446_Ofilm_TM_White_V02_D01_20180122_app.i */
                FTS_INFO("vendor id match, get fw file successfully, %s display maker ID: 0x%x",
                            "TM", r_buf[1]);
                break;
            } else if (r_buf[1] == 0x37) {
                /* upgrade C3C_FT5446_Ofilm_EBBG_White_V02_D01_20180122_app.i */
                fw = &fw_list[1];
                FTS_INFO("vendor id match, get fw file successfully, %s display maker ID: 0x%x",
                            "EBBG", r_buf[1]);
                break;
