...
#endif
{
	...
	case CAM_CALIOC_S_WRITE:	/*Note: Write Command is Unverified! */
		...
		totalLength = ptempbuf->u4Offset + ptempbuf->u4Length;

		switch (ptempbuf->deviceID) {
		case DUAL_CAMERA_MAIN_SENSOR: /* MAIN = 0x01 */
				if (g_eepromMainData != NULL) {
					if (g_eepromMainData->dataBuffer && totalLength <= g_eepromMainData->dataLength) {
						...
						PK_ERR("maybe some error buf(%p), read(%d), have(%d)\n",
										g_eepromMainData->dataBuffer, totalLength,
										g_eepromMainData->dataLength);
					}
				}
				break;
		case DUAL_CAMERA_SUB_SENSOR: /* SUB = 0x02 */
				if (g_eepromSubData != NULL) {
					if (g_eepromSubData->dataBuffer && totalLength <= g_eepromSubData->dataLength) {
						...
						PK_ERR("maybe some error buf(%p), read(%d), have(%d)\n",
										g_eepromSubData->dataBuffer, totalLength,
										g_eepromSubData->dataLength);
					}
				}
				break;
		case DUAL_CAMERA_MAIN_2_SENSOR: /* MAIN_2 = 0x04 */
				if (g_eepromMain2Data != NULL) {
					if (g_eepromMain2Data->dataBuffer && totalLength <= g_eepromMain2Data->dataLength) {
						...
						PK_ERR("maybe some error buf(%p), read(%d), have(%d)\n",
										g_eepromMain2Data->dataBuffer, totalLength,
										g_eepromMain2Data->dataLength);
					}
				}
				break;
		default:
	         ...
	         const char *buf, size_t count)
{
    ...
    if(totalBuf == NULL) {
        ...
        int retValue = 0;
		checkSum = 0;
        retValue = dual_cam_cal_read_total_data(sensorID,DUAL_CAMERA_MAIN_SENSOR,totalBuf);
